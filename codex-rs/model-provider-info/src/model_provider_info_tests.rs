...
fn test_copilot_provider_is_registered_as_builtin() {
    ...
    assert!(providers.contains_key(COPILOT_PROVIDER_ID));
    let copilot = &providers[COPILOT_PROVIDER_ID];
    assert_eq!(copilot.name, "GitHub Copilot");
    assert!(!copilot.requires_openai_auth);
    assert!(!copilot.supports_websockets);
    assert_eq!(copilot.wire_api, WireApi::Responses);
    assert_eq!(copilot.base_url, None);
    assert_eq!(
        copilot.http_headers.as_ref(),
        Some(&maplit::hashmap! {
