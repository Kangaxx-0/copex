...
fn resolve_copilot_provider(provider: &mut ModelProviderInfo) -> anyhow::Result<()> {
    use crate::copilot_auth::fetch_copilot_user_info;
    use crate::copilot_auth::read_copilot_token_from_store;
    use codex_keyring_store::DefaultKeyringStore;

    let store = DefaultKeyringStore;
    let token = read_copilot_token_from_store(&store, /*github_username*/ None)?;

    let handle = tokio::runtime::Handle::current();
    let user_info =
        tokio::task::block_in_place(|| handle.block_on(fetch_copilot_user_info(&token)))?;

    tracing::info!(
        login = %user_info.login,
        plan = %user_info.copilot_plan,
        endpoint = %user_info.api_endpoint,
        quota_entitlement = user_info.quota.as_ref().map(|quota| quota.entitlement),
        quota_remaining = user_info.quota.as_ref().map(|quota| quota.remaining),
        quota_percent_remaining = user_info.quota.as_ref().map(|quota| quota.percent_remaining),
        quota_unlimited = user_info.quota.as_ref().map(|quota| quota.unlimited),
